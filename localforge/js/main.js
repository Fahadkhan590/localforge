...
(function () {
  ...
  function applyServiceParam() {
    ...
    // Initial selection from URL only — never re-apply after user edits
    if (needSelect.getAttribute("data-service-applied") === "1") return;
    ...
    needSelect.setAttribute("data-service-applied", "1");
    needSelect.addEventListener("change", function () {
      needSelect.setAttribute("data-user-changed", "1");
    });

    // Clean query string; keep full pathname for nested-safe URLs
